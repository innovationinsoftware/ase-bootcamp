...
import { SEMATTRS_EXCEPTION_MESSAGE, SEMATTRS_EXCEPTION_TYPE, SEMATTRS_HTTP_HOST, SEMATTRS_HTTP_METHOD, SEMATTRS_HTTP_SCHEME, SEMATTRS_HTTP_STATUS_CODE, SEMATTRS_HTTP_TARGET, SEMATTRS_HTTP_URL, SEMATTRS_NET_PEER_IP, SEMATTRS_NET_PEER_NAME, SEMATTRS_NET_PEER_PORT, SEMATTRS_RPC_GRPC_STATUS_CODE, SEMRESATTRS_K8S_CRONJOB_NAME, SEMRESATTRS_K8S_DAEMONSET_NAME, SEMRESATTRS_K8S_DEPLOYMENT_NAME, SEMRESATTRS_K8S_JOB_NAME, SEMRESATTRS_K8S_POD_NAME, SEMRESATTRS_K8S_REPLICASET_NAME, SEMRESATTRS_K8S_STATEFULSET_NAME, SEMRESATTRS_SERVICE_INSTANCE_ID, SEMRESATTRS_SERVICE_NAME, SEMRESATTRS_SERVICE_NAMESPACE, SEMRESATTRS_TELEMETRY_SDK_VERSION, } from "@opentelemetry/semantic-conventions";
...
export function resourceMetricsToQuickpulseDataPoint(metrics, baseMonitoringDataPoint, documents) {
    const metricPoints = [];
    ...
    const quickpulseDataPoint = Object.assign(Object.assign({}, baseMonitoringDataPoint), { timestamp: new Date(), metrics: metricPoints, documents: documents });
...
export function getSpanDocument(span) {
    let document = {
...
export function getLogDocument(logRecord) {
    let document = {
...
function createPropertiesFromAttributes(attributes) {
    const properties = [];
    if (attributes) {
