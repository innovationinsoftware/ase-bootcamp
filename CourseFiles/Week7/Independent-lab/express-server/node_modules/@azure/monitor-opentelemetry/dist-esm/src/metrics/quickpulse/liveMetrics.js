...
import { getCloudRole, getCloudRoleInstance, getLogDocument, getSdkVersion, getSpanDocument, getTransmissionTime, } from "./utils";
...
export class LiveMetrics {
    ...
    activateMetrics(options) {
        ...
        this.memoryCommitedGauge = this.meter.createObservableGauge(QuickPulseOpenTelemetryMetricNames.COMMITTED_BYTES, {
            valueType: ValueType.INT,
        });
        this.processorTimeGauge = this.meter.createObservableGauge(QuickPulseOpenTelemetryMetricNames.PROCESSOR_TIME, {
            valueType: ValueType.DOUBLE,
        });
    ...
    recordSpan(span) {
        if (this.isCollectingData) {
            ...
            const document = getSpanDocument(span);
    ...
    recordLog(logRecord) {
        if (this.isCollectingData) {
            const document = getLogDocument(logRecord);
            this.addDocument(document);
    ...
    getCommitedMemory(observableResult) {
